...
export default class FindrouteControl extends IDEE.impl.Control {
  ...
  constructor(osrmurl, osrmurlAlternativa) {
    super();
    if (IDEE.utils.isUndefined(osrmurl)) {
  ...
  addTo(map, html) {
    ...
    this.utils = {
      getNearest: (coord) => {
        ...
        return new Promise((resolve, reject) => {
          ...
          IDEE.proxy(false);
          IDEE.remote.get(url + coord4326.join()).then((response) => {
            if(!IDEE.utils.isUndefined(response.code) && response.code != 200){
      ...
      createFeature: (coord, id) => {
        let feature = new IDEE.Feature("marcador" + id, {
          "id": id
        });
        let geojsonObject = {
          'type': 'Point',
      ...
      createRoute: (polyline, tipoRuta) => {
        ...
        var featureRuta = new IDEE.Feature("ruta" + Math.random(), {
          type: "Feature",
          properties: {},
          geometry: {
            type: "LineString",
    ...
    this.marcadorLayer = new IDEE.layer.GeoJSON({
      source: {
    ...
    //Capa para las rutas
    this.rutaLayer_ = new IDEE.layer.Vector({
      name: 'Ruta',
      extract: false
    });

    //Capa para los inicios de los steps
    this.stepLayer_ = new IDEE.layer.Vector({
  ...
  activateClick(map) {
    ...
    if(IDEE.impl.Map.Z_INDEX[IDEE.layer.type.WFS] == 1){
        ...
        this.marcadorLayer.setZIndex(IDEE.impl.Map.Z_INDEX[IDEE.layer.type.WFS] + 152);
        this.rutaLayer_.setZIndex(IDEE.impl.Map.Z_INDEX[IDEE.layer.type.WFS] + 153);
        this.stepLayer_.setZIndex(IDEE.impl.Map.Z_INDEX[IDEE.layer.type.WFS] + 154);
    }  
  ...
  addPoint(evt, result, inputAct) {
      ...
      if(!IDEE.utils.isUndefined(result)){
      ...
      this.utils.getNearest(coordinate).then((coord_geo) => {
        ...
        for (let i = 0, ilen = points_length; i < ilen; i += 1) {
          if(IDEE.utils.isNullOrEmpty(control.points[i])){
            emptyValues = true;
          }else{
        ...
        Promise.all(promiseArray).then((routeResult) => {
          if (!IDEE.utils.isNullOrEmpty(routeResult)) {
            ...
            IDEE.dialog.show("No se ha podido calcular la ruta: " + err, "Error", 'error');
          });

      }).catch((err) => {
        IDEE.dialog.show("No se ha podido obtener el punto más cercano: " + err, "Error", 'error');
      });
  ...
  calculateRoute(point1, point2) {
    return new Promise((resolve, reject) => {
      ...
      const reqRuta = IDEE.utils.addParameters(url + point1 + ';' + point2, {
        alternatives: '2',
        steps: 'true',
        annotations: 'true',
        geometries: 'polyline',
      ...
      IDEE.proxy(false);
      IDEE.remote.get(reqRuta).then((response) => {
        if(!IDEE.utils.isUndefined(response.code) && response.code != 200){
          reject(response.code);
        }else{
          let json = JSON.parse(response.text);
  ...
  addStepFeature(evt, polyline) {
    ...
    var initialFeature = new IDEE.Feature("step" + Math.random(), {
      "type": "Feature",
      "geometry": {
        "type": "Point",
