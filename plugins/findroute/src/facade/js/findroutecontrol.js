...
export default class FindrouteControl extends IDEE.Control {
   ...
   * @api stable
   */
  constructor(url, osrmurl, osrmurlAlternativa, conflictedPlugins, urlGeocoderInverso, searchstreetNormalizar) {
    // 1. checks if the implementation can create PluginControl
    if (IDEE.utils.isUndefined(FindrouteImplControl)) {
      IDEE.exception('La implementación usada no puede crear controles findrouteControl');
    }
    ...
    this.searchstreetNormalizar = searchstreetNormalizar;
   ...
   */
   createView(map) {
    ...
    return new Promise((success, fail) => {
      const html = IDEE.template.compileSync(templatefindroute);
      this.addEvents(html);
      success(html);
    });
   }
  ...
  search(query, processor) {
    ...
    const normalizar = IDEE.utils.addParameters(this.searchstreetNormalizar, {
      cadena: query,
    });
  ...
  showResults(results) {
    ...
    const html = IDEE.template.compileSync(templatefindrouteresults, options);
    ...
    if (!IDEE.utils.isNullOrEmpty(resultado)) {
      ...
      if (docs.geocoderMunProvSrsReturn instanceof Array) {
        if (!IDEE.utils.isUndefined(docs.geocoderMunProvSrsReturn[0].coordinateX)) {
          for (let i = 0, ilen = docs.geocoderMunProvSrsReturn.length; i < ilen; i += 1) {
            ...
            docs.geocoderMunProvSrsReturn[i].streetType =
              IDEE.utils.beautifyString(docs.geocoderMunProvSrsReturn[i].streetType);
            docs.geocoderMunProvSrsReturn[i].streetName =
              IDEE.utils.beautifyString(docs.geocoderMunProvSrsReturn[i].streetName);
          }
          resultsTemplateVar = {
        ...
        docs.geocoderMunProvSrsReturn.streetType =
          IDEE.utils.beautifyString(docs.geocoderMunProvSrsReturn.streetType);
        docs.geocoderMunProvSrsReturn.streetName =
          IDEE.utils.beautifyString(docs.geocoderMunProvSrsReturn.streetName);
        resultsTemplateVar = {
          docs: [docs.geocoderMunProvSrsReturn],
          containtResult,
          search,
  ...
  showRouteResults(routeResults) {
    ...
    for(let j = 0; j < routes.length; j++) {
      ...
      const options = { jsonp: true, vars: resultsTemplateVars };
      const html = IDEE.template.compileSync(templatefindrouteresultsroute, options);
  ...
  inverseGeocoderSearch(coordinates, input, processor) {
    ((searchTime) => {
      ...
      const geocoderInverso = IDEE.utils.addParameters(this.urlGeocoderInverso, {
        x: coord23030[0],
        y: coord23030[1],
        srs: this.facadeMap_.getProjection().code
      });
      IDEE.proxy(true);
      IDEE.remote.get(geocoderInverso).then((response) => {
        IDEE.proxy(false);
        if (searchTime === this.searchTime_) {
          let results;
          try {
  ...
  showGeocoderInvResults(results) {
    if(!IDEE.utils.isNullOrEmpty(results.streetName)){
  ...
  addInput() {
    ...
    for (let i = elementsLast.length - 1; i >= 0; i -= 1) {
...
FindrouteControl.createFormElement = function(label, type, id, classes, title, placeholder) {
  ...
  if (!IDEE.utils.isNullOrEmpty(type)) {
  ...
  if (!IDEE.utils.isNullOrEmpty(classes)) {
  ...
  if (!IDEE.utils.isNullOrEmpty(placeholder)) {
