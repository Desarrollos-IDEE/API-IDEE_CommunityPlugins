...
export default class ToponomysearchControl extends IDEE.Control {
  ...
  constructor() {
    ...
    if (IDEE.utils.isUndefined(ToponomysearchImplControl)) {
      IDEE.exception('La implementación usada no puede crear controles ToponomysearchControl');
    }
  ...
  */
 checkButtons() {
  this.clearBtn_.disabled = (IDEE.utils.isNullOrEmpty(this.selectProvince_.value) && IDEE.utils.isNullOrEmpty(this.inputMunicipality_.value)
    && IDEE.utils.isNullOrEmpty(this.inputName_.value));
...
checkSearch(evt) {
  if (evt.keyCode === 13) {
    if (evt.target.value.trim().length >= 3) {
      ...
      IDEE.dialog.error('Debe introducir una búsqueda de al menos 3 caracteres.');
    }
  }
  this.checkButtons();
}
...
*/
clearInputs() {
  if (!IDEE.utils.isNullOrEmpty(this.inputName_)) {
    this.inputName_.value = '';
  }
  if (!IDEE.utils.isNullOrEmpty(this.selectProvince_)) {
    this.selectProvince_.value = '';
  }
  if (!IDEE.utils.isNullOrEmpty(this.selectProvince_)) {
    this.inputMunicipality_.value = '';
  }
  if (!IDEE.utils.isNullOrEmpty(this.optionsInputs_)) {
...
search() {
  ...
  let provinceFilter = (IDEE.utils.isNullOrEmpty(this.selectProvince_.value)) ? '' : '<PropertyIsEqualTo><PropertyName>app:provincia</PropertyName><Literal>' + this.encodeHTML_(this.selectProvince_.value) + '</Literal></PropertyIsEqualTo>';
  ...
  let searchUrl = IDEE.utils.addParameters(this.searchUrl_, {
    'VERSION': '1.1.0',
    'SERVICE': 'WFS',
    'REQUEST': 'GetFeature',
  ...
  IDEE.remote.get(searchUrl).then((response) => {
    ...
    if (localTime === this.searchTime_) {
      ...
      xml2js.parseString(response.text, { explicitArray: false, normalizeTags: true, tagNameProcessors: [this.removeXMLTags_] }, (err, result) => {
        ...
        if (!IDEE.utils.isNullOrEmpty(this.results_) && !Array.isArray(this.results_)) {
          this.results_ = [this.results_];
        }
        this.showResults_();
        if (!IDEE.utils.isNullOrEmpty(this.results_)) {
 ...
 */
showResults_() {
  const html = IDEE.template.compileSync(templateresults, {
    'jsonp': true,
