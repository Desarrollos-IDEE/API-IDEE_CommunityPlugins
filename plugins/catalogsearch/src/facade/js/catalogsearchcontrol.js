...
export default class CatalogSearchControl extends IDEE.Control {
  /**
  ...
  constructor(geoNetworkUrl) {
    ...
    if (IDEE.utils.isUndefined(CatalogSearchImplControl)) {
      IDEE.exception('La implementación usada no puede crear controles CatalogSearchControl');
    }
  ...
  createView(map) {
    ...
    const promise = new Promise((success, fail) => {
      const html = IDEE.template.compileSync(catalogsearch, { jsonp: true });
      this.addEvents(html);
      success(html);
    });
    return promise;
  ...
  addEvents(html) {
    ...
    IDEE.utils.enableTouchScroll(this.resultsContainer_);
  ...
  searchClick_(evt) {
    ...
    if ((evt.type !== 'keyup') || (evt.keyCode === 13)) {
      ...
      if (IDEE.utils.isNullOrEmpty(query)) {
        IDEE.dialog.info('Debe introducir una búsqueda.');
      } else {
        this.search_(query);
      }
    }
  ...
  searchUuid(uuid, searchTime) {
    return new Promise((success, fail) => {
      const searchUrl = IDEE.utils.addParameters(`${this.geoNetworkUrl_}/xml.metadata.get`, {
        uuid,
      });
            ...
            if (metadata.type === 'dataset' || metadata.type === 'series') {
              ...
              if (Array.isArray(metadataWMSUrl)) {
                for (let j = 0; j < (metadataWMSUrl.length - 1); j += 1) {
                  ...
                  if (!IDEE.utils.isNullOrEmpty(data) && data.trim() === 'OGC:WMS-1.3.0-http-get-capabilities') {
              ...
              if (Array.isArray(metadataWMSUrl)) {
                for (let j = 0; j < (metadataWMSUrl.length - 1); j += 1) {
                  ...
                  if (!IDEE.utils.isNullOrEmpty(service) && service.trim() === 'GetCapabilities') {
  ...
  showLayers_(url, layers, varLayersContainer) {
    ...
    const varlayers = (!IDEE.utils.isNullOrEmpty(layers.Layer)) ? layers.Layer : layers;
    const html = IDEE.template.compileSync(catalogsearchlayers, {
      jsonp: true,
  ...
  addLayersControl(evt) {
    ...
    for (let i = 0; i < listLayersElement.length; i += 1) {
      ...
      if (listLayersElement[i].checked) {
        this.facadeMap_.addWMS(new IDEE.layer.WMS({
          url,
  ...
  showFullMetadata(evt) {
    ...
    if (IDEE.utils.isNullOrEmpty(metadataContainer.innerHTML)) {
      IDEE.remote.get(url).then((response) => {
        ...
        this.element_.classList.remove(CatalogSearchControl.SEARCHING_CLASS);
        IDEE.dialog.error('No se ha podido parsear el metadato completo');
      });
    } else {
      this.element_.classList.remove(CatalogSearchControl.SEARCHING_CLASS);
    }
   ...
   */
  showResults_() {
    const html = IDEE.template.compileSync(catalogsearchresults, {
      jsonp: true,
      vars: this.results_,
    });
  ...
  clearClick_(evt) {
    ...
    if (!IDEE.utils.isNullOrEmpty(this.input_)) {
      this.input_.value = '';
    }
    if (!IDEE.utils.isNullOrEmpty(this.resultsContainer_)) {
